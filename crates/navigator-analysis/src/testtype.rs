...
pub fn coverage_profile_from_bai(bam_path: &Path, mean_read_length: Option<u64>) -> Option<CoverageProfile> {
    ...
    let depth = |reads: u64, len: u64| if len > 0 { (reads * read_len) as f64 / len as f64 } else { 0.0 };
...
pub fn infer_test_type(
    ...
    let y_ratio = if p.autosome_depth > 0.0 { p.y_depth / p.autosome_depth } else { f64::INFINITY };
...
mod tests {
    ...
    fn prof(a: f64, y: f64, m: f64, has_auto: bool) -> CoverageProfile {
        CoverageProfile { autosome_depth: a, y_depth: y, mt_depth: m, has_autosomes: has_auto }
    }
    ...
    fn targeted_y_maps_vendor_or_generic() {
        ...
        assert_eq!(infer_test_type(Some(&p), Some("ILLUMINA"), Some("FamilyTreeDNA"), None).as_deref(), Some("BIG_Y_700"));
        assert_eq!(infer_test_type(Some(&p), Some("ILLUMINA"), Some("Full Genomes"), None).as_deref(), Some("Y_ELITE"));
        assert_eq!(infer_test_type(Some(&p), Some("ILLUMINA"), Some("YSEQ"), None).as_deref(), Some("Y_PRIME"));
        // Unknown vendor → TARGETED_Y, not a mislabeled Big Y.
        assert_eq!(infer_test_type(Some(&p), Some("ILLUMINA"), None, None).as_deref(), Some("TARGETED_Y"));
    }
    ...
    fn targeted_y_by_enrichment_with_offtarget_autosomes() {
        ...
        assert_eq!(infer_test_type(Some(&p), None, Some("Full Genomes"), None).as_deref(), Some("Y_ELITE"));
        assert_eq!(infer_test_type(Some(&p), None, None, None).as_deref(), Some("TARGETED_Y"));
    }

    #[test]
    fn targeted_mt_only_when_autosomes_absent() {
        let p = prof(0.0, 0.0, 800.0, false);
        assert_eq!(infer_test_type(Some(&p), Some("ILLUMINA"), None, None).as_deref(), Some("MT_FULL_SEQUENCE"));
    }
    ...
    fn wgs_not_mislabeled_by_high_copy_mt_or_modest_y() {
        ...
        assert_eq!(infer_test_type(Some(&male), Some("ILLUMINA"), None, None).as_deref(), Some("WGS"));
        // Female WGS: y≈0, mt high — the autosome-present guard keeps it WGS, not targeted-MT.
        let female = prof(30.0, 0.02, 1200.0, true);
        assert_eq!(infer_test_type(Some(&female), Some("ILLUMINA"), None, None).as_deref(), Some("WGS"));
    }
    ...
    fn wgs_when_autosomes_covered() {
        ...
        assert_eq!(infer_test_type(Some(&hifi), Some("PACBIO"), None, None).as_deref(), Some("WGS_HIFI"));
        // Long read by length, no platform string.
        assert_eq!(infer_test_type(Some(&hifi), None, None, Some(15000)).as_deref(), Some("WGS_HIFI"));
    }
    ...
    fn no_profile_falls_back_to_platform() {
        assert_eq!(infer_test_type(None, Some("PACBIO"), None, None).as_deref(), Some("WGS_HIFI"));
        assert_eq!(infer_test_type(None, Some("ILLUMINA"), None, None).as_deref(), Some("WGS"));
