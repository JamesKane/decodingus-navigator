...
async fn download_once(
    ...
    let http_err = |source: reqwest::Error| RefgenomeError::Http {
        url: url.to_string(),
        source,
    };
    ...
    if let Some(parent) = dest.parent() {
        tokio::fs::create_dir_all(parent)
            .await
            .map_err(|e| RefgenomeError::io(parent, e))?;
    }
    ...
    let mut file = tokio::fs::File::create(&part)
        .await
        .map_err(|e| RefgenomeError::io(&part, e))?;
    ...
    tokio::fs::rename(&part, dest)
        .await
        .map_err(|e| RefgenomeError::io(dest, e))?;
