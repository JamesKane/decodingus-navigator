...
object EntityConversions:
  ...
  def toMatchConsentEntity(consent: MatchConsent, biosampleId: UUID): MatchConsentEntity =
    MatchConsentEntity(
      ...
      biosampleId = biosampleId,
      consentLevel = consent.consentLevel.toDbString,
      allowedMatchTypes = consent.allowedMatchTypes.map(_.toDbString),
  ...
  def fromMatchConsentEntity(entity: MatchConsentEntity, biosampleRef: String): MatchConsent =
    MatchConsent(
      ...
      meta = fromEntityMeta(entity.meta),
      biosampleRef = biosampleRef,
      consentLevel = ConsentLevel.fromString(entity.consentLevel),
      allowedMatchTypes = entity.allowedMatchTypes.map(MatchType.fromString),
  ...
  def toMatchRequestEntity(request: MatchRequest): MatchRequestEntity =
    MatchRequestEntity(
      ...
      requestType = request.requestType.toDbString,
  ...
  def fromMatchRequestEntity(entity: MatchRequestEntity): MatchRequest =
    MatchRequest(
      ...
      status = RequestStatus.fromString(entity.status),
      requestType = RequestType.fromString(entity.requestType),
  ...
  def toMatchResultEntity(result: MatchResult, biosampleId: UUID): MatchResultEntity =
    MatchResultEntity(
      ...
      relationshipEstimate = result.relationshipEstimate.map(_.toDbString),
  ...
  def fromMatchResultEntity(entity: MatchResultEntity, biosampleRef: String): MatchResult =
    MatchResult(
      ...
      relationshipEstimate = entity.relationshipEstimate.map(RelationshipEstimate.fromString),
