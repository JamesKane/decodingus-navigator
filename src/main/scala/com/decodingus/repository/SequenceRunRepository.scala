...
class SequenceRunRepository extends SyncableRepositoryBase[SequenceRunEntity]:
  ...
  override def insert(entity: SequenceRunEntity)(using conn: Connection): SequenceRunEntity =
    ...
    executeUpdate(
      """INSERT INTO sequence_run (
        ...
        |  run_fingerprint, total_reads, pf_reads, pf_reads_aligned,
        |  pct_pf_reads_aligned, reads_paired, pct_reads_paired, pct_proper_pairs,
        |  read_length, max_read_length, mean_insert_size, median_insert_size, std_insert_size,
        |  pair_orientation, flowcell_id, run_date,
      ...
      Seq(
        ...
        entity.pctPfReadsAligned,
        entity.readsPaired,
        entity.pctReadsPaired,
        entity.pctProperPairs,
        entity.readLength,
        entity.maxReadLength,
        entity.meanInsertSize,
        entity.medianInsertSize,
        entity.stdInsertSize,
        entity.pairOrientation,
        entity.flowcellId,
  ...
  override def update(entity: SequenceRunEntity)(using conn: Connection): SequenceRunEntity =
    ...
    executeUpdate(
      """UPDATE sequence_run SET
        ...
        |  total_reads = ?, pf_reads = ?, pf_reads_aligned = ?,
        |  pct_pf_reads_aligned = ?, reads_paired = ?, pct_reads_paired = ?, pct_proper_pairs = ?,
        |  read_length = ?, max_read_length = ?,
        |  mean_insert_size = ?, median_insert_size = ?, std_insert_size = ?,
        |  pair_orientation = ?, flowcell_id = ?, run_date = ?, files = ?,
  ...
  override protected def mapRow(rs: ResultSet): SequenceRunEntity =
    ...
    SequenceRunEntity(
      ...
      pctPfReadsAligned = getOptDouble(rs, "pct_pf_reads_aligned"),
      readsPaired = getOptLong(rs, "reads_paired"),
      pctReadsPaired = getOptDouble(rs, "pct_reads_paired"),
      pctProperPairs = getOptDouble(rs, "pct_proper_pairs"),
      readLength = getOptInt(rs, "read_length"),
      maxReadLength = getOptInt(rs, "max_read_length"),
      ...
      pairOrientation = getOptString(rs, "pair_orientation"),
