...
class SyncConflictRepository:
  ...
  def insert(entity: SyncConflictEntity)(using conn: Connection): SyncConflictEntity =
    executeUpdate(
      ...
      Seq(
        ...
        entity.localChanges.map(_.noSpaces).orNull,
        entity.remoteChanges.map(_.noSpaces).orNull,
        entity.overlappingFields.map(_.noSpaces).orNull,
        ...
        entity.localSnapshot.map(_.noSpaces).orNull,
        entity.remoteSnapshot.map(_.noSpaces).orNull,
        entity.createdAt,
        entity.updatedAt
      )
    )
  ...
  private def mapRow(rs: ResultSet): SyncConflictEntity =
    SyncConflictEntity(
      ...
      localChanges = getOptString(rs, "local_changes").flatMap(parse(_).toOption),
      remoteChanges = getOptString(rs, "remote_changes").flatMap(parse(_).toOption),
      overlappingFields = getOptString(rs, "overlapping_fields").flatMap(parse(_).toOption),
      ...
      localSnapshot = getOptString(rs, "local_snapshot").flatMap(parse(_).toOption),
      remoteSnapshot = getOptString(rs, "remote_snapshot").flatMap(parse(_).toOption),
