...
| Canonical `chm13v2.0_maskedY_rCRS` analysis reference | 🟢 | **Built 2026-06-10.** `Build::Chm13v2MaskedRcrs` — resolvable + cached (pull-once-use-many) via the existing Build-keyed gateway (`chm13v2.0_maskedY_rCRS.fa`). `Build::nuclear()` normalizes it to `Chm13v2` for liftover, so it reuses CHM13's chains (identical nuclear coords); chrM is a direct rCRS query (skips the rotation map). Best-effort probe detection via the `@PG CL`/`@SQ UR` reference filename. Unit-tested. *Not yet auto-preferred for calling — Navigator consumes pre-aligned BAMs, so it's selected by the alignment's stamped build, not substituted at analysis time. No masked-aligned BAM on hand to validate end-to-end.* |
| Reference-polarity metadata + guard (CHM13 Y = HG002 = J) | 🟢 | **Built 2026-06-10.** `Build::reference_polarity() → ReferencePolarity` records each reference's chrY/chrM provenance + the invariant (CHM13 Y = HG002 = J → reference base is *derived*; polarity comes from the tree, never REF/ALT). The placement path was already polarity-safe (`haplo::locus_state` compares the sample's base to the tree's explicit anc/der, never the reference); added a regression guard test (`reference_polarity_comes_from_the_tree_not_the_reference`) that encodes the J-Y trap and would fail if a refactor reintroduced REF-as-ancestral. |
