...
- **Realignment** — rebuild a GRCh38 or GRCh37 whole genome against **CHM13v2 (hs1)**. No human reference was complete before the Telomere-to-Telomere project: GRCh37 and GRCh38 both leave whole stretches blank, the Y worst of all. T2T closed those gaps — a full autosomal sequence end to end, plus the first complete Y chromosome — from a donor of European ancestry whose Y belongs to the J1a lineage, which is worth stating because a complete genome is still one person's genome. The gain is genome-wide; Navigator's use for it today is Y discovery, where on a validated 30× sample realigning took the share of chrY receiving any coverage from 41% to 98%, with Y and mtDNA haplogroups matching the same donor's independently produced CHM13 alignments. Opt-in and never automatic — hours of work and hundreds of GB of scratch, checked for up front — it never modifies the source file, and an interrupted run resumes from where it stopped
